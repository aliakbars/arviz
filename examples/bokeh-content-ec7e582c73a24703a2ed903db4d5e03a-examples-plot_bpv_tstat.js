(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("ef56f255-e431-4a42-b178-8a88cb89f544");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'ef56f255-e431-4a42-b178-8a88cb89f544' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.2.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"c3b88b8c-75c3-4aad-a223-1db18235e0ee":{"defs":[],"roots":{"references":[{"attributes":{},"id":"17910","type":"SaveTool"},{"attributes":{},"id":"17939","type":"AllLabels"},{"attributes":{},"id":"17945","type":"UnionRenderers"},{"attributes":{},"id":"17909","type":"UndoTool"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"17933"}]},"id":"17950","type":"LegendItem"},{"attributes":{"data":{},"selected":{"id":"17946"},"selection_policy":{"id":"17945"}},"id":"17929","type":"ColumnDataSource"},{"attributes":{},"id":"17888","type":"DataRange1d"},{"attributes":{"toolbars":[{"id":"17914"}],"tools":[{"id":"17904"},{"id":"17905"},{"id":"17906"},{"id":"17907"},{"id":"17908"},{"id":"17909"},{"id":"17910"},{"id":"17911"}]},"id":"17976","type":"ProxyToolbar"},{"attributes":{"source":{"id":"17923"}},"id":"17928","type":"CDSView"},{"attributes":{"source":{"id":"17951"}},"id":"17956","type":"CDSView"},{"attributes":{},"id":"17905","type":"PanTool"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"17950"}]},"id":"17949","type":"Legend"},{"attributes":{"below":[{"id":"17896"}],"center":[{"id":"17899"},{"id":"17903"},{"id":"17949"}],"height":500,"left":[{"id":"17900"}],"output_backend":"webgl","renderers":[{"id":"17927"},{"id":"17933"},{"id":"17955"}],"title":{"id":"17957"},"toolbar":{"id":"17914"},"toolbar_location":null,"width":500,"x_range":{"id":"17888"},"x_scale":{"id":"17892"},"y_range":{"id":"17890"},"y_scale":{"id":"17894"}},"id":"17887","subtype":"Figure","type":"Plot"},{"attributes":{"coordinates":null,"data_source":{"id":"17929"},"glyph":{"id":"17930"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17932"},"nonselection_glyph":{"id":"17931"},"view":{"id":"17934"}},"id":"17933","type":"GlyphRenderer"},{"attributes":{"children":[[{"id":"17887"},0,0]]},"id":"17975","type":"GridBox"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17953","type":"Circle"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17952","type":"Circle"},{"attributes":{},"id":"17946","type":"Selection"},{"attributes":{"source":{"id":"17929"}},"id":"17934","type":"CDSView"},{"attributes":{"callback":null},"id":"17911","type":"HoverTool"},{"attributes":{"axis":{"id":"17900"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17903","type":"Grid"},{"attributes":{},"id":"17901","type":"BasicTicker"},{"attributes":{},"id":"17892","type":"LinearScale"},{"attributes":{},"id":"17894","type":"LinearScale"},{"attributes":{"coordinates":null,"formatter":{"id":"17938"},"group":null,"major_label_policy":{"id":"17939"},"ticker":{"id":"17901"}},"id":"17900","type":"LinearAxis"},{"attributes":{"data":{},"selected":{"id":"17971"},"selection_policy":{"id":"17970"}},"id":"17951","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"data_source":{"id":"17951"},"glyph":{"id":"17952"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17954"},"nonselection_glyph":{"id":"17953"},"view":{"id":"17956"}},"id":"17955","type":"GlyphRenderer"},{"attributes":{"toolbar":{"id":"17976"},"toolbar_location":"above"},"id":"17977","type":"ToolbarBox"},{"attributes":{"overlay":{"id":"17912"}},"id":"17906","type":"BoxZoomTool"},{"attributes":{},"id":"17890","type":"DataRange1d"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17930","type":"Line"},{"attributes":{},"id":"17938","type":"BasicTickFormatter"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17924","type":"Line"},{"attributes":{},"id":"17943","type":"UnionRenderers"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17954","type":"Circle"},{"attributes":{},"id":"17942","type":"AllLabels"},{"attributes":{},"id":"17944","type":"Selection"},{"attributes":{"coordinates":null,"data_source":{"id":"17923"},"glyph":{"id":"17924"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17926"},"nonselection_glyph":{"id":"17925"},"view":{"id":"17928"}},"id":"17927","type":"GlyphRenderer"},{"attributes":{"axis":{"id":"17896"},"coordinates":null,"group":null,"ticker":null},"id":"17899","type":"Grid"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17925","type":"Line"},{"attributes":{},"id":"17897","type":"BasicTicker"},{"attributes":{},"id":"17907","type":"WheelZoomTool"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17944"},"selection_policy":{"id":"17943"}},"id":"17923","type":"ColumnDataSource"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17926","type":"Line"},{"attributes":{},"id":"17904","type":"ResetTool"},{"attributes":{"children":[{"id":"17977"},{"id":"17975"}]},"id":"17978","type":"Column"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17912","type":"BoxAnnotation"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17932","type":"Line"},{"attributes":{},"id":"17970","type":"UnionRenderers"},{"attributes":{},"id":"17971","type":"Selection"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17931","type":"Line"},{"attributes":{"coordinates":null,"formatter":{"id":"17941"},"group":null,"major_label_policy":{"id":"17942"},"ticker":{"id":"17897"}},"id":"17896","type":"LinearAxis"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17913","type":"PolyAnnotation"},{"attributes":{},"id":"17941","type":"BasicTickFormatter"},{"attributes":{"tools":[{"id":"17904"},{"id":"17905"},{"id":"17906"},{"id":"17907"},{"id":"17908"},{"id":"17909"},{"id":"17910"},{"id":"17911"}]},"id":"17914","type":"Toolbar"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17957","type":"Title"},{"attributes":{"overlay":{"id":"17913"}},"id":"17908","type":"LassoSelectTool"}],"root_ids":["17978"]},"title":"Bokeh Application","version":"2.4.2"}}';
                  const render_items = [{"docid":"c3b88b8c-75c3-4aad-a223-1db18235e0ee","root_ids":["17978"],"roots":{"17978":"ef56f255-e431-4a42-b178-8a88cb89f544"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();